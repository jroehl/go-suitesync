...
func Sync(src string, dest string, bidirectional bool) (al []string, ar []string, dl []string, dr []string, err error) {
	rh := getRemoteHash(filepath.Clean(filepath.Join(dest, lib.Credentials[lib.HashFile])))
	if err := lib.CheckDir(src); err != nil {
		return nil, nil, nil, nil, err
	}

	if len(rh) < 1 {
		lib.PrNoticeF("Initializing new sync dir in \"%s\":\n", dest)
...
func UploadRestlet(path string) {
	deployProject(path)
...
func UploadFiles(root string, files []string, dest string) []string {
	...
	for _, f := range files {
		...
		if _, err := lib.CheckExists(sf); err != nil {
			log.Fatal(err)
		}

		nfb := filepath.Clean(pr.Filebase)
		nrd := strings.Replace(dest, lib.Credentials[lib.RootPath], "", -1)
...
func DownloadFiles(paths []string, dest string) (srcs []string, dests []string) {
	...
	for _, p := range paths {
		rel := strings.Replace(p, lib.Credentials[lib.RootPath], "", -1)
		src := filepath.Join(strings.Replace(pr.Filebase, lib.Credentials[lib.RootPath], "", -1), p)
...
func UpdateHashFile(src string, dest string, exclude bool, failed []string) (f []lib.Hash) {
	...
	nd := strings.Replace(dest, lib.Credentials[lib.RootPath], "", -1)
	tmp := lib.MkTempDir()
	th := filepath.Clean(filepath.Join(tmp, "tmp-hash"))
	d := filepath.Clean(filepath.Join(th, nd))
	hf := filepath.Clean(filepath.Join(d, lib.Credentials[lib.HashFile]))
	...
	restlet.Delete([]string{filepath.Clean(filepath.Join("", dest, lib.Credentials[lib.HashFile]))})
